...
func BindJSONWithValidate(c *gin.Context, dto interface{}, tagName string) {
	...
	v, _ := common.SingletonValidator(tagName)
...
func handleValidateErr(c *gin.Context, err error, tagName string) {
	...
	if errs, ok := err.(validator.ValidationErrors); ok {
		message = fmt.Sprintf("%v", errs.Translate(common.GetZHTrans(tagName)))
	} else {
		message = err.Error()
	}
...
func GetOne(entity interface{}) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if has, _ := dao.Engine().ID(id).Get(entity); has == true {
...
func Create(entity interface{}) gin.HandlerFunc {
	return func(c *gin.Context) {
		BindJSON(c, entity)
		if _, err := dao.Engine().Insert(entity); err != nil {
			log.Println(err)
...
func Update(entity interface{}, persist interface{}) gin.HandlerFunc {
	return func(c *gin.Context) {
		BindJSON(c, entity)
		if has, _ := dao.Engine().ID(c.Params.ByName("id")).Get(persist); has == false {
			c.JSON(http.StatusBadRequest, gin.H{"message": "id 对应的数据不存在"})
			return
		}
		dao.Engine().ID(c.Params.ByName("id")).Update(entity)
		c.JSON(http.StatusOK, nil)
	}
}
...
func Delete(entity interface{}) gin.HandlerFunc {
	return func(c *gin.Context) {
		dao.Engine().ID(c.Params.ByName("id")).Delete(entity)
		c.JSON(http.StatusOK, nil)
	}
}
